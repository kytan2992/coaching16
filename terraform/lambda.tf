data "archive_file" "create_url" {
  type        = "zip"
  source_dir  = "./lambda_functions/create_url"
  output_path = "./lambda_functions/create_url.zip"  
}

data "archive_file" "retrieve_url" {
  type        = "zip"
  source_dir  = "./lambda_functions/retrieve_url"
  output_path = "./lambda_functions/retrieve_url.zip"  
}
...
resource "aws_lambda_function" "create_url" {
  ...
  filename      = "./lambda_functions/create_url.zip"
...
resource "aws_lambda_function" "retrieve_url" {
  ...
  runtime       = "python3.13"
  role          = aws_iam_role.lambda_role.arn
  filename      = "./lambda_functions/retrieve_url.zip"
